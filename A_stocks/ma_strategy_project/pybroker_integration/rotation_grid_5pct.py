...
def _daily_pct_band_hit(
    ...
    """用日线高低点近似判断：当日涨幅区间是否覆盖 [pct_min, pct_max]。"""
    if prev_close <= 0:
        return False
    low_pct = (low - prev_close) / prev_close * 100.0
    high_pct = (high - prev_close) / prev_close * 100.0
    if low_pct != low_pct or high_pct != high_pct:
        return False
    return max(low_pct, pct_min) <= min(high_pct, pct_max)
    ...
    *,
    ...
    for dt in dates:
        ...
        for sym, r in rows.items():
            ...
            ma = _to_float(r["ma20"])
            prev_close = _to_float(r.get("prev_close", float("nan")))
            if ma <= 0 or not (low <= buy_p <= high):
                continue
            if not _daily_pct_band_hit(low, high, prev_close, MORNING_PCT_MIN, MORNING_PCT_MAX):
                continue
            amp = (ma - low) / ma
    ...
    return {
        "strategy": "rotation_grid_ma20_morning_pct",
        ...
        "morning_pct_min": MORNING_PCT_MIN,
        "morning_pct_max": MORNING_PCT_MAX,
...
def main() -> None:
    ...
    print(f"rotation_grid_ma20_morning_pct  区间: {start_date} ~ {end_date}")
    ...
    print(f"额外约束: 日内涨幅区间覆盖 {MORNING_PCT_MIN:.2f}% ~ {MORNING_PCT_MAX:.2f}%（日线近似）")
