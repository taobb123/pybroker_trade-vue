...
按 ROC 降序取前 10，输出到控制台作为「收益率 / 动量」维度的因子暴露参考。
...
def main() -> None:
    ...
    print('=' * 92)
    print('收益率因子暴露（60 日 ROC 截面 | 统一最近交易日）')
    print(f"截面交易日: {asof_ts.date()}  |  股票池: {len(symbols)}，有效参与排名: {len(rows)}，输出 Top {TOP_N}")
    print('=' * 92)
    print(f"{'排名':<6}{'代码':<10}{'名称':<14}{'ROC60':>12}{'暴露权重(组内)':>18}")
    print('-' * 92)
